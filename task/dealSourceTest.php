...
while (true) {
    try{
        $i++;
        //$res = $queueList->setDb($mysql)->getSourceQueue(2);
        $tableName = "source_add_queue";
        $sql = "select * from {$tableName} where id={$a} limit 1";
        $res=$mysql->doSql($sql); 
        if (empty($res)) {
            sleep(1);
            if($i>3){
        ...
        foreach ($res as $key => $value) {file_put_contents('./s.txt',$value['id'].' '.time(),FILE_APPEND);
            ...
            if ($info['type'] == 11) {
                ...
                foreach ($queueList->getFileLine($value['path'],$tmpStart,$tmpTotalLines) as $va) {
                    try{
                        ...
                        if (empty($exists)) {
                            $tmpCurData['content'] =  addslashes(htmlspecialchars($vaContent));
